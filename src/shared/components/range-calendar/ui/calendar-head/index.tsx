...
export const CalendarHead: React.FC = () => {
    ...
    const handlePrev = () => {
        changeDate(date.subtract(1, "month").format("YYYY-MM-DD"));
    };

    const handleNext = () => {
        changeDate(date.add(1, "month").format("YYYY-MM-DD"));
    };

    return (
        <div className={css.header}>
            <button className={`${css.arrow} ${css.arrowPrev}`} onClick={handlePrev}>
            ...
            <button className={`${css.arrow} ${css.arrowNext}`} onClick={handleNext}>
                <img src={arrowSvg.src} alt="" />
            </button>
        </div>
    );
