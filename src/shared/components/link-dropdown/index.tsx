...
export const LinkDropdown: React.FC<Props> = ({ name, links, solid }) => {
    ...
    return (
        <div
            className={clsx(css.root, active && css.rootActive, solid && css.rootSolid)}
            onMouseEnter={!solid ? onActive : undefined}
            onMouseLeave={!solid ? onDeactivate : undefined}
            ref={ref}
            data-link
        >
            <button className={css.button} onClick={active ? onDeactivate : onActive}>
                <span>{name}</span>
                <strong />
            </button>
                ...
                <div className={css.dropdown}>
                    <div className={css.dropdown_wrapper}>
                        <div className={css.dropdown_list}>
                            {links.map((item) => (
                                <li className={css.dropdown_item} key={item.url}>
                                    {item.url === "/" ? (
                                        ...
                                        >
                                            ...
                                            >
                                                <span
                                                    style={{ fontWeight: item.bold ? "600" : "" }}
                                                >
                                                    {item.name}
                                                    {item.url.startsWith("/") ? (
                                                        ""
