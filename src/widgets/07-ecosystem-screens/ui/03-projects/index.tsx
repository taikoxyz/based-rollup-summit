...
export const Projects: React.FC = () => {
    ...
    const { data, fetchNextPage, hasNextPage, isFetching } = useInfiniteQuery<IProjectsResponse>({
        queryKey: [EcosystemApiEnum.ALL_PROJECTS, getProjectsQuery(router.query, filters)],
        initialPageParam: 0,
        queryFn: ({ pageParam }) =>
            projectApi.getAll(
    ...
    const projects = useMemo(() => flatProjects(data?.pages || []), [data]);
            ...
            <div className="container">
                <div className={css.wrapper}>
                    ...
                    <div className={css.disclaimer}>
                        <strong>Disclaimer:</strong> Be cautious when using third-party
                        applications. Applications listed are provided for information without
                        recommendation or endorsement.{" "}
                        <strong>Your security and privacy are important</strong>, so research
                        thoroughly before engaging with third-party applications. We are not
                        responsible for any risks or losses associated with the use of any
                        third-party offerings. Proceed at your own discretion.
                    </div>
                </div>
            </div>
        </section>
