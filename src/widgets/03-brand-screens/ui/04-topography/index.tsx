...
export const Topography: React.FC = () => {
    const topography = useTranslationObject<IBrandTopography[]>("topography.list", "brand-assets");
    const fonts = useTranslationObject<IBrandFont[]>("topography.fonts", "brand-assets");
                ...
                <div className={css.wrapper}>
                    <div className={css.left}>
                        <div className={css.card}>
                            <h3 className={css.suptitle}>{t("topography.suptitle")}</h3>

                            <h2 className={css.title}>{t("topography.title")}</h2>

                            <ul className={css.list}>
                                {topography.map((item, id) => (
                                    <li className={css.list_item} key={item.title}>
                                        <p className={css.list_index}>{id + 1}</p>
                    ...
                    <div className={css.right}>
                        <ul className={css.fonts}>
                            {fonts.map((font) => (
                                <li className={css.fonts_item} key={font.fontFamily}>
                                    <div className={css.fonts_up}>
                                        <h4 className={css.fonts_suptitle}>{font.title}</h4>

                                        <h3 className={css.fonts_family}>{font.fontFamily}</h3>

                                        <div className={css.fonts_example}>
                                            <span />
                                            <img className={css.fonts_aa} src={font.font} alt="" />
                                        </div>
                                    </div>
