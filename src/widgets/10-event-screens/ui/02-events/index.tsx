...
import {
    eventGetMonths,
    eventTransformLocations,
    getEventQuery,
} from "widgets/10-event-screens/lib";
...
export const Events: React.FC = () => {
    ...
    const { data } = useQuery<IEventsResponse>({
        queryKey: [EventsApiEnum.ALL_EVENTS, getEventQuery(filter)],
        queryFn: () => eventApi.getAll(getEventQuery(filter)),
    });

    useEffect(() => {
        eventApi.getLocations().then((data) => setLocations(data));
    }, []);

    const locationOptions = useMemo(() => eventTransformLocations(locations), [locations]);

    const monthOptions = useMemo(() => eventGetMonths(), []);
                    ...
                    <div className={css.right}>
                        <div className={css.head}>
                            <MediaQuery
                                ...
                                children={<p className={css.head_event}>{t("eventName")}</p>}
                            />
                            <div className={css.head_420}>
                                <div className={css.head_column}>
                                    <TaikoSelect
                                        ...
                                        onChange={(data) =>
                                            data && setFilterValue("location", data)
                                        }
                                    />
                                </div>
                                <div className={css.head_column}>
                            ...
                            <MediaQuery
                                query="(min-width: 769px)"
                                children={<div className={css.head_controls} />}
                            />
                        </div>
