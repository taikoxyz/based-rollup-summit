...
import { flatBlogs, getBlogQuery } from "widgets/08-blog-screens/lib";
...
export const Blogs: React.FC = () => {
    ...
    const { data, hasNextPage, fetchNextPage, isFetching } = useInfiniteQuery<IBlogsResponse>({
        queryKey: [BlogApiKeys.ALL_BLOGS, query],
        queryFn: ({ pageParam }) =>
            blogApi.getAll(getBlogQuery(router.query, pageParam as number, state)),
        initialPageParam: 0,
    ...
    const blogs = useMemo(() => flatBlogs(data?.pages || []), [data]);
