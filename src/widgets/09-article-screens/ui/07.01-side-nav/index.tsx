...
export const SideNav: React.FC = () => {
    ...
    return (
        <div className={css.nav}>
            <p className={css.nav_title}>{t("contentTitle")}</p>

            <nav className={css.nav_container}>
                {getArticleLinks(content).map((link) => (
                    <a className={css.nav_item} href={`#${link.hash}`} key={link.name + link.hash}>
                        <span>{link.name}</span>
                    </a>
                ))}
                <a className={css.nav_item} href={`#${ArticleScreensEnum.HOW_TO_APPLY}`}>
