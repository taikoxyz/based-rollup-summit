...
import { IBlogsNear } from "shared/lib/types";
...
export const Links: React.FC = () => {
    ...
    useEffect(() => {
        blogApi.near(article.id).then((blog) => setBlogs(blog));
    }, [article]);

    return (
        <section className={css.links}>
            <MediaQuery query="(max-width: 768px)" children={<Share />} />
                ...
                <ul className={css.links_list}>
                    ...
                    {blogs.next && (
                        <li className={`${css.links_item} ${css.links_itemNext}`}>
                            <h4 className={css.links_title}>
                                <span>{t("next")}</span>
                                <Sprite.Default icon="arrow-right-small" />
                            </h4>
